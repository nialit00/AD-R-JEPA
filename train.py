...
def run_model(model, loss_fn, map_seg_loss_fn, d, device='cuda:0', sw=None, use_radar_encoder=None,
    ...
    if not (unwrap_model(model).use_radar if hasattr(model, "module") else model.use_radar):
        in_occ_mem0 = None
    elif unwrap_model(model).use_radar and (unwrap_model(model).use_metaradar or use_shallow_metadata):
    ...
    if train_task == 'both' or train_task == 'map':
        ...
        #   map
        fc_map_factor = 1 / torch.exp(unwrap_model(model).fc_map_weight)
        ...
        metrics['fc_map_weight'] = unwrap_model(model).fc_map_weight.item()
        # single threshold IoUs (t=0.4)
    ...
    if train_task == 'both' or train_task == 'object':
        ...
        # obj
        ce_factor = 1 / torch.exp(unwrap_model(model).ce_weight)
        ...
        metrics['ce_weight'] = unwrap_model(model).ce_weight.item()
    ...
    if sw is not None and sw.save_this:
        if unwrap_model(model).use_radar:
            rad_occ_mem0 = vox_util.voxelize_xyz(rad_xyz_cam0, Z, Y, X, assert_cube=False)
...
def main(
    ...
    if "device_ids" in config:
        print(f"[WARN] Overriding YAML device_ids: {config['device_ids']}")

    config.pop("device_ids", None)
    device_ids = list(range(num_gpus)) if num_gpus > 0 else [0]

    # FORCE SINGLE GPU MODE (recommended for your setup)
    if num_gpus > 1 and os.environ.get("FORCE_SINGLE_GPU", "1") == "1":
        device_ids = [0]

    print(f"[INFO] device_ids = {device_ids}")
    ...
    if len(device_ids) > 1:
        assert (B % len(device_ids) == 0), \
            f"Batch size {B} must be divisible by {len(device_ids)} GPUs"
    ...
    if init_dir:
        if load_step and load_optimizer and load_scheduler:
            ...
            global_step = saverloader.load(init_dir, unwrap_model(model), optimizer, ignore_load=ignore_load,
                                           device_ids=device_ids)
            print("global_step: ", global_step)
        elif load_step:
            global_step = saverloader.load(init_dir, unwrap_model(model), ignore_load=ignore_load, device_ids=device_ids)
        else:
            _ = saverloader.load(init_dir, unwrap_model(model), ignore_load=ignore_load)
            global_step = 0
    ...
    while global_step < max_iters:
        ...
        if np.mod(global_step, save_freq) == 0:
            ...
            saverloader.save(ckpt_dir, optimizer, unwrap_model(model), global_step, scheduler=scheduler,
                             keep_latest=keep_latest)
