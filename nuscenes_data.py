...
from utils.radar_utils import get_radar_data as radar_fn
...
class NuscData(torch.utils.data.Dataset):
    def __init__(self, nusc: NuScenes, nusc_maps: dict, is_train: bool, data_aug_conf: dict,
        ...
        self.vox_util = utils.vox.Vox_util(
            Z, Y, X,
            scene_centroid=torch.from_numpy(np.array(self.centroid, dtype=np.float32)).float().cuda(),
            bounds=self.bounds,
            assert_cube=False)
